...
namespace Aetherwave
{
    ...
    public class ImageDisplayManager : MonoBehaviour
    {
        ...
        private int skyboxClearFrames = 0; // Counter to clear skybox multiple times

        void Start()
        {
            Debug.Log("🎨 ImageDisplayManager starting...");
            
            // FORCE remove any scene-level skybox material first
            RenderSettings.skybox = null;
            DynamicGI.UpdateEnvironment();
            Debug.Log("🎨 Scene skybox material cleared");

            // Ensure camera is configured for solid color background
            Camera mainCamera = Camera.main;
            if (mainCamera != null)
            {
                // Force solid color mode and disable skybox completely
                mainCamera.clearFlags = CameraClearFlags.SolidColor;
                mainCamera.backgroundColor = Color.black; // Initial color
                
                Debug.Log("✅ Camera configured for solid color background, skybox disabled");
            }
            else
            {
                Debug.LogWarning("⚠️ No main camera found for background configuration");
            }

            if (imageDisplay == null)
            {
                Debug.Log("🔍 imageDisplay is null, trying to find RawImage component...");
                imageDisplay = GetComponent<RawImage>();                if (imageDisplay == null)
                {
        ...
        void Update()
        {
            // AGGRESSIVE skybox clearing for first 60 frames
            if (skyboxClearFrames < 60)
            {
                RenderSettings.skybox = null;
                skyboxClearFrames++;
                if (skyboxClearFrames == 60)
                {
                    Debug.Log("🎨 Skybox aggressively cleared for 60 frames");
                }
            }
        ...
        void ApplyImageBasedBackgroundColor(Texture2D texture)
        {
            if (texture == null)
            {
                Debug.LogError("🎨 ApplyImageBasedBackgroundColor: texture is null");
                return;
            }

            Debug.Log($"🎨 Starting background color extraction for texture: {texture.width}x{texture.height}");

            try
            {
                // Ensure texture is readable for pixel sampling
                Texture2D readableTexture = MakeTextureReadable(texture);
                Debug.Log($"🎨 Made texture readable: {readableTexture.width}x{readableTexture.height}");

                // Sample colors from the edges of the image to get a good background color
                Color backgroundColor = ExtractDominantEdgeColor(readableTexture);
                Debug.Log($"🎨 Extracted raw edge color: R={backgroundColor.r:F3} G={backgroundColor.g:F3} B={backgroundColor.b:F3}");

                // Darken and desaturate the color for a subtle background
                backgroundColor = Color.Lerp(backgroundColor, Color.black, 0.75f); // Darken more
                backgroundColor = Color.Lerp(backgroundColor, new Color(backgroundColor.grayscale, backgroundColor.grayscale, backgroundColor.grayscale), 0.4f); // More desaturate
                Debug.Log($"🎨 Processed background color: R={backgroundColor.r:F3} G={backgroundColor.g:F3} B={backgroundColor.b:F3}");

                // Apply to camera and ensure skybox is disabled
                Camera mainCamera = Camera.main;
                if (mainCamera != null)
                {
                    // Force clear flags and disable skybox every time
                    mainCamera.clearFlags = CameraClearFlags.SolidColor;
                    mainCamera.backgroundColor = backgroundColor;
                    RenderSettings.skybox = null;
                    DynamicGI.UpdateEnvironment();

                    Debug.Log($"🎨 Applied image-based background color: #{ColorUtility.ToHtmlStringRGB(backgroundColor)} from image, skybox disabled");
                    Debug.Log($"🎨 Camera background now set to: R={mainCamera.backgroundColor.r:F3} G={mainCamera.backgroundColor.g:F3} B={mainCamera.backgroundColor.b:F3}");
                }
                else
                {
                    Debug.LogError("🎨 No main camera found for background color application!");
                }
            ...
            {
                Debug.LogWarning($"⚠️ Could not extract background color from image: {e.Message}");
                ...
                if (mainCamera != null)
                {
                    ...
                    RenderSettings.skybox = null;
                    Debug.Log("🎨 Applied fallback background color: dark gray");
                }
            }
        }
        ...
        Texture2D MakeTextureReadable(Texture2D source)
        {
            ...
            {
                // Need to create a readable copy
